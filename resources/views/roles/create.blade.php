...
<div class="page-header mb-4">
    <h1 class="fw-bold mb-1"><i class="fas fa-plus-circle me-2" style="color:#e74c3c"></i>Crear Nuevo Rol</h1>
</div>
<div class="card border-0 shadow-sm"><div class="card-body p-4">
    <form action="{{ route('roles.store') }}" method="POST">@csrf
        ...
        <input type="text" name="nombre_rol" class="form-control" value="{{ old('nombre_rol') }}" placeholder="Ej: Recepcionista"></div>
        ...
        <input class="form-check-input" type="checkbox" name="permisos[]" value="{{ $key }}" id="p_{{ $key }}">
        <label class="form-check-label" for="p_{{ $key }}">{{ $label }}</label>
        </div></div>@endforeach</div></div>
        <div class="d-flex gap-2"><button type="submit" class="btn btn-danger">Guardar Rol</button>
